...
class SFA:
    ...
    def get_EB(self, data):
        return np.mat((np.dot(data.T, data) / (data.shape[0] - 1)))

    def get_ZB(self, diff_data):
        return np.mat((np.dot(diff_data.T, diff_data) / (diff_data.shape[0] - 1)))
    ...
    def fit(self, data_original):
        """
        This function is used for training SFA model
    ...
    def transform(self, data_original):
        """
        This function is used to analyze the test data by using the trained SFA model
...
def find_kde(input, Confidence):
    ...
    """
    plt.figure()
    ax = sns.kdeplot(input, cumulative=True, bw=((input.max() - input.min()) / 960))
